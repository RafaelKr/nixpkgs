...
- The NetBird server module gained a native relay, an embedded identity provider and a reworked ingress, which changes several options.
  `services.netbird.server.enableNginx` and its per-component counterparts under `dashboard`, `management` and `signal` are replaced by `services.netbird.server.ingress`, a tagged union that selects exactly one ingress backend.
  Set `services.netbird.server.ingress.nginx.enable = true` and move virtual-host settings to `services.netbird.server.ingress.nginx.settings`.
  The old options still work through a compatibility shim, but they now enable one ingress for every enabled component, so the per-component opt-out is gone.
  `services.netbird.server.management.settings.DataStoreEncryptionKey` is now mandatory because it previously defaulted to a publicly known placeholder.
  Generate a key with `openssl rand -base64 32` and pass it as `{ _secret = "/path/to/file"; }`.
  The placeholder `TURNConfig` credentials are no longer written, for the same reason.
  `services.netbird.server.management.disableSingleAccountMode` has been removed in favour of `services.netbird.server.management.singleAccountMode.enable`, and `services.netbird.server.management.singleAccountModeDomain` is now `services.netbird.server.management.singleAccountMode.domain`.
